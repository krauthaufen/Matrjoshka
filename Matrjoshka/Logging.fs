...
type MultiLogger(loggers : list<ILogger>) =
    interface ILogger with
        member x.WriteLine (line) =
            loggers |> List.iter (fun l -> l.WriteLine line)

        member x.WriteColoredLine color line =
            loggers |> List.iter (fun l -> l.WriteColoredLine color line)
...
[<AutoOpen>]
module Logging =
    let private real = MultiLogger [ConsoleLogger() :> ILogger; MonitorableHtmlLogger(9998) :> ILogger ] :> ILogger
